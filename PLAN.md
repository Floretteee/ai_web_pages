...
### 3.1 统一构建口径
- **现状**：`README.md` 写“无任何构建工具依赖”，但 `package.json` 依赖 `esbuild`，并运行 `build` 脚本打包 `vendor/domd`。
- **目标**：
  - `README.md` 修正为“核心运行时无框架依赖，构建阶段使用 esbuild 处理 domd”。
  - 或把 `build` 流程扩展为：压缩 CSS/JS、生成带 hash 的资源、自动同步 SW 版本号。

### 3.2 清理未使用依赖
- **现状**：`dependencies` 包含 `react`、`react-dom`、`immer`、`nanoid`，但应用代码未使用。
- **目标**：
  - 若 `domd` 构建确实不需要这些依赖，则从 `dependencies` 移除。
  - 若未来计划使用，应明确引入或移入 `devDependencies`。
...
### 4.2 指数退避重试
- **现状**：重试固定 2 秒延迟。
- **目标**：`delay = min(2^retry * 1000, 8000)`，首次 1s、第二次 2s、第三次 4s，最多 8s。
